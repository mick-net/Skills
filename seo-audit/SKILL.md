...
**Next.js App Router readiness** (when applicable)
- Inspect the rendered output and the owning implementation: `app/robots.ts` or public `robots.txt`, `app/sitemap.ts` or sitemap generator, and route metadata/`generateMetadata` canonicals
- Require absolute canonical URLs that match the preferred protocol, host, path, and trailing-slash policy
- Keep sitemaps limited to canonical, public, successful, indexable URLs; exclude auth, preview, parameter, redirect, duplicate, and noindex routes
- Confirm robots references the production sitemap and does not accidentally block required public assets or routes
- Verify generated output in the running production-like build; source-file presence alone is not proof
...
For priority pages, compare the page against what Google is actually rewarding, not only against generic on-page best practices:

- Dominant page type and format: tool, guide, comparison, directory, category page, forum thread, video, news result, or local page.
- SERP features: AI Overview/AI Mode, People Also Ask, map pack, images, video, shopping, sitelinks, dates, forums/UGC, or news.
- Authority gap: root authority, page-level referring domains, brand trust, high-authority publishers, marketplaces, government/medical/finance/YMYL sites, or entrenched UGC.
- Consensus coverage: common entities, subtopics, questions, proof, schema, freshness, media, and structure across ranking pages.
- Information-gain edge: original data, examples, comparison, tool, benchmark, template, updated figure, niche proof, or clearer extractable structure.

If the SERP is blocked by authority or the wrong page type, say that directly. Do not frame a title rewrite, word-count increase, or generic E-E-A-T polish as sufficient to rank.

### Business Value & Conversion Check

For priority organic pages, audit whether search visibility can plausibly turn into business value:

- Search signal vs proof: Search Console impressions, clicks, rankings, volume, and CPC show demand or visibility, not payment proof.
- Paid boundary: page has a clear next action such as signup, trial, checkout, demo, email capture, paid audit, paid pilot, quote request, or high-intent follow-up.
- Qualified conversion: analytics, PostHog, Clarity, CRM, Stripe/payment data, or manual notes show whether organic visitors become qualified users or buyers.
- Money-page focus: if one commercial page is already getting impressions but underperforming, prefer a focused sprint on promise, title/H1, proof, screenshots, CTA, speed, internal links, one supporting asset, and targeted mentions.
- Free-tool fit: free tools should solve a searched job, demonstrate paid value, stay cheap to maintain, and lead to upgrade, capture, or a paid/manual offer.
- pSEO fit: programmatic or template pages need unique data, examples, templates, integrations, inventory, or user-generated value. Start with a small measured batch before scaling.

If a page ranks but does not attract qualified action, say whether to improve the page, change the CTA, merge it, noindex it, prune it, or keep it only as a trust/support asset.
...
- Highest-value page or asset to fix first
...
**Business-Value SEO Findings**
For each priority page or asset:
- **Search signal**: impressions/clicks/rankings/volume/CPC evidence
- **Qualified action**: signup, trial, checkout, demo, capture, paid audit, paid pilot, etc.
- **Revenue proof gap**: what is missing from analytics/payment evidence
- **Fix**: page, CTA, asset, internal-link, pruning, or measurement change
